...
class UiRelativeLayoutLayout @JvmOverloads constructor(
    context: Context,
...
) : RelativeLayout(context, attrs, defStyleAttr)
{
    ...
    init {
        attrs?.let {
            val typedArray = context.obtainStyledAttributes(it, R.styleable.UiRelativeLayoutLayout)
            cornerRadius = typedArray.getDimension(R.styleable.UiRelativeLayoutLayout_cornerRadius, 0f)
            stWidth = typedArray.getDimension(R.styleable.UiRelativeLayoutLayout_strokeWidth, 0f)
            stColorDark = typedArray.getColor(R.styleable.UiRelativeLayoutLayout_stColorDark, Color.BLACK)
            stColorLight = typedArray.getColor(R.styleable.UiRelativeLayoutLayout_stColorLight, Color.BLACK)
            bgColorDark = typedArray.getColor(R.styleable.UiRelativeLayoutLayout_bgColorDark, Color.TRANSPARENT)
            bgColorLight =
                typedArray.getColor(R.styleable.UiRelativeLayoutLayout_bgColorLight, Color.TRANSPARENT)
            bgGradientStart =
                typedArray.getColor(R.styleable.UiRelativeLayoutLayout_bgGradientStart, Color.TRANSPARENT)
            bgGradientCenter =
                typedArray.getColor(R.styleable.UiRelativeLayoutLayout_bgGradientCenter, Color.TRANSPARENT)
            bgGradientEnd =
                typedArray.getColor(R.styleable.UiRelativeLayoutLayout_bgGradientEnd, Color.TRANSPARENT)
            isGradient = bgGradientStart != Color.TRANSPARENT && bgGradientEnd != Color.TRANSPARENT
            isDistance = typedArray.getBoolean(R.styleable.UiRelativeLayoutLayout_strokeDistance, false)
            distanceSpace = typedArray.getDimension(R.styleable.UiRelativeLayoutLayout_distanceSpace, 10f)

            val gradient = typedArray.getString(R.styleable.UiRelativeLayoutLayout_strokeGradient)
            ...
            gradientOrientation =
                when (typedArray.getInt(R.styleable.UiRelativeLayoutLayout_bgGdOrientation, 0)) {
            ...
            strokeGradientOrientation =
                when (typedArray.getInt(R.styleable.UiRelativeLayoutLayout_strokeGdOrientation, 6)) {
    ...
    private fun updateGradient() {
        val backgroundDrawable = GradientDrawable(
            ...
            cornerRadius = this@UiRelativeLayoutLayout.cornerRadius
        }
    ...
    override fun dispatchDraw(canvas: Canvas) {
        ...
        if (stWidth > 0) {
            val paint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
                ...
                strokeWidth = this@UiRelativeLayoutLayout.stWidth
                isAntiAlias = true
                isDither = true
