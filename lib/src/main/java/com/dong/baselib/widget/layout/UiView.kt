...
) : View(context, attrs, defStyleAttr)
{
    ...
    init {
        attrs?.let {
            val typedArray = context.obtainStyledAttributes(it, R.styleable.UiView)
            cornerRadius = typedArray.getDimension(R.styleable.UiView_cornerRadius, 0f)
            stWidth = typedArray.getDimension(R.styleable.UiView_strokeWidth, 0f)
            stColorDark = typedArray.getColor(R.styleable.UiView_stColorDark, Color.BLACK)
            stColorLight = typedArray.getColor(R.styleable.UiView_stColorLight, Color.BLACK)
            bgColorDark = typedArray.getColor(R.styleable.UiView_bgColorDark, Color.TRANSPARENT)
            bgColorLight =
                typedArray.getColor(R.styleable.UiView_bgColorLight, Color.TRANSPARENT)
            bgGradientStart =
                typedArray.getColor(R.styleable.UiView_bgGradientStart, Color.TRANSPARENT)
            bgGradientCenter =
                typedArray.getColor(R.styleable.UiView_bgGradientCenter, Color.TRANSPARENT)
            bgGradientEnd =
                typedArray.getColor(R.styleable.UiView_bgGradientEnd, Color.TRANSPARENT)
            isGradient = bgGradientStart != Color.TRANSPARENT && bgGradientEnd != Color.TRANSPARENT
            isDistance = typedArray.getBoolean(R.styleable.UiView_strokeDistance, false)
            distanceSpace = typedArray.getDimension(R.styleable.UiView_distanceSpace, 10f)

            val gradient = typedArray.getString(R.styleable.UiView_strokeGradient)
            ...
            gradientOrientation =
                when (typedArray.getInt(R.styleable.UiView_bgGdOrientation, 0)) {
            ...
            strokeGradientOrientation =
                when (typedArray.getInt(R.styleable.UiView_strokeGdOrientation, 6)) {
    ...
    private fun updateGradient() {
        val backgroundDrawable = GradientDrawable(
            ...
            cornerRadius = this@UiView.cornerRadius
        }
    ...
    override fun dispatchDraw(canvas: Canvas) {
        ...
        if (stWidth > 0) {
            val paint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
                ...
                strokeWidth = this@UiView.stWidth
                isAntiAlias = true
                isDither = true
