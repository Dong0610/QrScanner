...
import net.blwsmartware.qrcodescanner.databinding.ItemReslutViewBinding
...
import net.blwsmartware.qrcodescanner.model.WifiModel
import java.net.URI
import java.net.URL
...
fun detectQrType(content: String): QrType {
    ...
    return when {
        ...
        content.startsWith("geo:") ||content.startsWith("GEO:") ||
...
fun AppCompatActivity.resultPhone(lineLayout: LinearLayout, text: String) {
    lineLayout.removeAllViews()
    val itemBinding = ItemReslutViewBinding.inflate(layoutInflater, lineLayout, false)
    with(itemBinding) {
        resultTitle.text = getString(R.string.number)
        resultContent.text = if(text.startsWith("tel:") || text.startsWith("TEL:")) text.removePrefix("tel:").removePrefix("TEL:") else text
    }
    lineLayout.addView(itemBinding.root)
}
